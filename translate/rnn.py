...
def multi_bidirectional_rnn(cells, inputs, time_pooling=None, pooling_avg=False,
                            initial_state_fw=None, initial_state_bw=None,
                            dtype=None, sequence_length=None, scope=None):
  ...
  for i, (cell_fw, cell_bw) in enumerate(cells):
    # Forward direction
    with vs.variable_scope('MultiBiRNN_FW_{}'.format(i)) as fw_scope:
      output_fw, output_state_fw = rnn(cell_fw, inputs, initial_state_fw, dtype,
                                       sequence_length, scope=fw_scope)

    # Backward direction
    with vs.variable_scope('MultiBiRNN_BW_{}'.format(i)) as bw_scope:
      tmp, output_state_bw = rnn(cell_bw, _reverse_seq(inputs, sequence_length),
                                 initial_state_bw, dtype, sequence_length, scope=bw_scope)
    output_bw = _reverse_seq(tmp, sequence_length)
    # Concat each of the forward/backward outputs
    inputs = [array_ops.concat(1, [fw, bw])
  ...
  return inputs, array_ops.concat(1, output_states_fw), array_ops.concat(1, output_states_bw)
