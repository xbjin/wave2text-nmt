...
def pretrain():
  ...
  with tf.Session(config=tf.ConfigProto(gpu_options=gpu_options)) as sess:
    ...
    dev_set = [read_data([FLAGS.src_dev_ids[i]], FLAGS.trg_dev_ids) 
                                                        for i in range(encoder_count)]
                                                                  
    train_set = [read_data([FLAGS.src_train_ids[i]], FLAGS.trg_train_ids, FLAGS.max_train_data_size) 
                                                        for i in range(encoder_count)] 
                                                        

    
    
    train_bucket_sizes = [[len(train_set[i][b]) for b in xrange(len(_buckets))] 
                                                        for i in range(encoder_count)]
    ...
    # TODO si on pick un bucket dans une langue, doit on prendre le même pour les autres langues (pour le step) ?  
                               
    random_number_01 = np.random.random_sample()
    bucket_id = min([i for i in xrange(len(train_buckets_scale[0]))
                   if train_buckets_scale[0][i] > random_number_01])                               
    ...
    for i in range(2):
        model = models[i]
        train_set_ = train_set[i]
    
      # Get a batch and make a step.
    
        start_time = time.time()
        encoder_inputs, decoder_inputs, target_weights = model.get_batch(
        train_set_, bucket_id)
